...
export function generateTypes(prompts: ChatPromptTemplate[], outputPath: string) {
    const lines: string[] = [
...
function jsonSchemaToType(schema: PromptOutputSchema, promptName: string, rootName = 'Root'): string {
    const parseSchema = (schema: PromptOutputSchema, name: string, increment: number): string => {
...
function jsonSchemaToZodType(schema: PromptOutputSchema, promptName: string, rootName = 'Root'): string {
    const parseSchema = (schema: PromptOutputSchema, name: string, increment: number): string => {
        if (schema.type === 'object' && schema.properties) {
            const required = schema.required || []
            const props = Object.entries(schema.properties)
